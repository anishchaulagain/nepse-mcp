...
@mcp.tool()
async def get_stock_candles_tool(symbol: str, timeframe: str = "1D") -> dict:
    ...
    """
    df = await get_stock_candles(symbol, timeframe)
    candles = []
...
@mcp.tool()
async def calculate_indicators_tool(symbol: str) -> dict:
    ...
    """
    result = await calculate_indicators(symbol)
    return result.model_dump()


@mcp.tool()
async def predict_trend_tool(symbol: str) -> dict:
    ...
    Args:
        symbol: Stock ticker symbol (e.g. NABIL)
    """
    result = await predict_trend(symbol)
    return result.model_dump()


@mcp.tool()
async def detect_breakout_tool(symbol: str) -> dict:
...
@mcp.tool()
async def analyze_stock_tool(symbol: str) -> dict:
